...
export default function PatientAppointments() {
  ...
  // Refresh the page state so the Join button becomes active when the appointment time arrives.
  useTimeRefresh()

  // Get the latest upcoming and past appointments when the page loads.
  useEffect(() => { dispatch(fetchPatientAppointments()) }, [dispatch])

  const handleDownload = (filename) => {
    // Create a temporary file for downloading the selected medical report.
          ...
          {upcoming.map((a) => (
            <div key={a.appointment_id} className="d-flex flex-wrap align-items-center justify-content-between gap-3 rounded-4 p-3 border" style={{ borderColor: '#f1f5f9' }}>
              ...
              <div className="d-flex align-items-center gap-3 flex-wrap">
                ...
                {/* Suggested appointments let the patient accept or reject the doctor's proposed time. */}
                {a.status?.toLowerCase() === 'suggested' ? (
                  ...
                  <>
                    ...
                    {/* The Join button is enabled only when the appointment is ready to start. */}
