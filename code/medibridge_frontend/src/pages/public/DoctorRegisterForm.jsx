...
export default function DoctorRegisterForm() {
  ...
  // Store all the registration details in one place so the form is easier to manage.
  ...
  // Reuse this handler for updating different fields in the form.
  const set = (k) => (e) => setF({ ...f, [k]: e.target.value })

  const submit = async (e) => {
    e.preventDefault()

    // Register the doctor first and continue only if the registration succeeds.
    const res = await dispatch(registerDoctor(f))
    if (registerDoctor.fulfilled.match(res)) {
      try {
        // Send a welcome email after the doctor's account has been created.
        ...
        // Email failure is logged without stopping the successful registration flow.
        console.error('Failed to send welcome email:', err); 
      } 

      // Take the newly registered doctor to the doctor dashboard.
      navigate('/doctor') 
    } 
  } 
