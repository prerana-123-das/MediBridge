...
export default function ForgotPasswordPage() {
  ...
  // Keeps track of the current request state so the UI can show loading, success, or error messages.
  const [status, setStatus] = useState({ loading: false, success: false, error: '' })

  const handleSubmit = async (e) => {
    e.preventDefault()
    // Show the loading state while the password reset request is being processed.
    setStatus({ loading: true, success: false, error: '' })
    
    try {
      // Send the entered email to the backend email service to start the reset process.
      ...
      // Display a success message when the backend confirms that the reset email was sent.
      if (data.success) { 
      ...
      // Handles cases where the email service cannot be reached.
